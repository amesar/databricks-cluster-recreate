...
class ClusterTransformer():
    def __init__(self, client, output_dir, use_cluster_id, cluster_create_sleep=5):
        self.client = client
        self.output_dir = output_dir
        self.use_cluster_id = use_cluster_id
        self.manifests = []
        self.duplicate_clusters = ClusterDirectoryManager()
        self.cluster_create_sleep = cluster_create_sleep
    ...
    def _process(self, cluster_id):
        ...
        # -- Create _create_cluster.sh
    
        opath = os.path.join(cluster_dir,"_create_cluster.sh")
        with open(opath, "w") as f:
            f.write("\n# Create cluster\n\n")
        ...
        with open(opath, "w") as f:
            ...
            f.write("_create_cluster.sh > cluster_id.json\n")
            f.write(f"sleep {self.cluster_create_sleep} # wait for cluster to be created\n")
            f.write("""cluster_id=`grep cluster_id cluster_id.json | sed -e 's/"//g' -e 's/cluster_id://' -e 's/ //g'` \n""")
            f.write("echo CLUSTER_ID: $cluster_id\n")
            f.write("PROFILE=$1\n")
            f.write("_install_libraries.sh $cluster_id $PROFILE\n")
        utils.chmod_x(opath)
